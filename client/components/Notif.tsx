...
export default class Notif extends Component<Props, State> {
  static get STATUS_CONSTRUCT() {
    return 1;
  }
  static get STATUS_DISPLAY() {
    return 2;
  }
  static get STATUS_START_OPEN() {
    return 3;
  }
  static get STATUS_START_NOTIF() {
    return 4;
  }
  static get STATUS_START_CLOSE() {
    return 5;
  }
  static get STATUS_UNDISPLAY() {
    return 6;
  }
  ...
  startNotif() {
    ...
    this.setState({
      ...
      style: {
        ...style,
        self: { ...style.self, transition: `${transition * 1}ms` }
      }
    });
  ...
  onTransitionEnd() {
    if (this.state.status === Notif.STATUS_START_CLOSE) {
      this.setState({ status: Notif.STATUS_UNDISPLAY });
    }
  }
  ...
  log(status) {
    ...
    switch (status) {
      case Notif.STATUS_CONSTRUCT:
        console.log(
          "@@@ 1 STATUS_CONSTRUCT " +
            this.state.style.self.transform +
            " " +
            this.state.style.self.transition +
            " " +
            date
        );
        break;
  ...
  render() {
    const { post, app, thread, handleOnClickToggleMain }: Props = this.props;
