...
export default class Window {
  ...
  public boot(): Promise<void> {
    return new Promise((resolve) => {
      ...
      if (this.id === define.APP_TYPES.TOP || this.id === define.APP_TYPES.PORTAL || this.id === define.APP_TYPES.EXTENSION) {
        // handle ext.
    ...
    if (currentTarget instanceof Worker) {
      if (type === PostMessage.WSAPI_TO_CLIENT_TYPE) {
        ...
        if (method === 'WS_CONSTRUCTED') {
          this.conned(this);
          if (this.id === define.APP_TYPES.TOP || this.id === define.APP_TYPES.PORTAL) {
            // @ts-ignore
  ...
  public wsClientAfterFilter({ method, params, state }) {
    switch (method) {
      ...
      case 'SERVER_TO_API[EMIT]:changeThread':
        if (this.window.id === define.APP_TYPES.TOP || this.window.id === define.APP_TYPES.PORTAL) {
          this.requestServer('searching', {
            // TODO: EXTで複数起動の場合に正しく動作するのか検証
            id: this.window.ext.id,
            ch: state.thread.ch,
